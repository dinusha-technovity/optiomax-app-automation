...
import com.Optiomax.pageObjects.AssetCategoryPage;
import com.Optiomax.pageObjects.BasePage;


public class AssetCategoryTest extends BasePage {

	private AssetCategoryPage assetCategoryPage;

	@BeforeClass
	public void setUp() throws InterruptedException {
		super.setup();
		assetCategoryPage = new AssetCategoryPage(driver);
		assetCategoryPage.login("sithum@gmail.com", "chamod1234");
		Thread.sleep(5000);
		driver.get("");
		Thread.sleep(5000);
	...
	public void testPageLoad() throws InterruptedException {
		Thread.sleep(5000);
		Assert.assertTrue(assetCategoryPage.isPageLoaded(), "The asset category page did not load successfully.");
	}
	
	@Test(priority = 2)
	public void testAssetCategoryCardsDisplay() throws InterruptedException {
		Assert.assertTrue(assetCategoryPage.getAssetCategoriesCardCount() > 0,
				"The asset category cards are not displayed.");
	}
	
	@Test(priority = 3)
	public void testAddAssetCategory() throws InterruptedException {
		assetCategoryPage.clickAssetCategory();
		Thread.sleep(3000);
		// Verify that the new pop-up window appears
		Assert.assertTrue(driver.findElement(By.xpath("//h3[normalize-space()='Add New Asset Categories']")).isDisplayed(),
				"The 'Add New Asset Category' popup did not appear.");
		assetCategoryPage.clickAssetCategoryClose();
	}
	
	@Test(priority = 4)
	public void testSearchFunctionaityWithValidAssetCategoryName() throws InterruptedException {
		assetCategoryPage.searchAssetManagement("Vehicle");
		Thread.sleep(3000);
		Assert.assertTrue(assetCategoryPage.isAssetCategoryPresent("Vehicle"), "The search functionality did not return the expected results.");
		Thread.sleep(3000);
		driver.navigate().refresh();
	}
